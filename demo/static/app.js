...
$("ref_file").addEventListener("change", () => {
...
async function stopRecording() {
  ...
  try {
    ...
    logStatus("Mic recording ready. Click Generate.");
  } catch (err) {
...
$("gen_btn").addEventListener("click", async () => {
  ...
  const ref_seconds = toMaybeNumber($("ref_seconds").value);
  const prefix_sec = toMaybeNumber($("prefix_sec").value);
  if (ref_seconds !== null) fd.append("ref_seconds", String(ref_seconds));
  if (prefix_sec !== null) fd.append("prefix_sec", String(prefix_sec));

  fd.append("anti_loop", String(!!$("anti_loop").checked));
  fd.append("use_prefix", String(!!$("use_prefix").checked));

  fd.append("use_stop_head", String(!!$("use_stop_head").checked));
  fd.append("stop_threshold", String(clampNumber($("stop_threshold").value, 0.0, 1.0, 0.8)));
  fd.append("stop_patience", String(clampNumber($("stop_patience").value, 1, 50, 5)));
